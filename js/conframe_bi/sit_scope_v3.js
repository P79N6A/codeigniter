...
var SIT_SCOPE = {
    ...
    'sms_icon' : 'fa fa-envelope',
    'email_icon' : 'fa fa-inbox',
    'ojur_icon' : 'fa fa-book',
...
SIT_SCOPE.init = function(){
    ...
    var html = '<div class="sit_cont" style="height:'+ ($(window).height()/3 - 27) +';">';
        html += '<div id="sit_cont"></div>';
        html += '<div id="refresh_sits"></div>';
    html += '</div>';
    $('#situation').html(html);

    SIT_SCOPE.load_data();

    if (SIT_SCOPE.situations != null && SIT_SCOPE.situations.length != 0){
 ...
 * Change Artem
 */
SIT_SCOPE.refresh_situations = function(){
    var key = String($('#txt_filter').val()).toUpperCase();
    var keys = key.split(',');
    var i = $('#sit_cont [sit_id]').length;
    if (i == 0){
        var html = '<div role="tooltip">';
            html += '<div id="pop_win" class="pop_window"></div>';
        html += '</div>';
        $('#sit_cont').append(html);
    }
    var j = $('#sit_cont [sit_id]').length;
    var n = $('#sit_cont [sit_id]').length+10;

    for (var k in keys){
        if (keys[k].length > 1){//неизвестно где используется
            i = 0;

            while (j < n && i < this.situations.length){
                if (String(this.situations[i]["text"]).toUpperCase().indexOf(keys[k]) != -1 && $('div[sit_id="'+this.situations[i].id+'"]').length == 0){
                    this.update_style(i);
                    this.add_situation(this.situations[i],i);
        ...
        else {//во время загрузки страницы или "посмотреть еще"

            var bits = check_filter_values();

            if (this.situations)
            while (j < n && i < this.situations.length){
                if (bits.ans == this.situations[i].is_auto_reg || bits.tns == this.situations[i].is_current || bits.zns == this.situations[i].is_finished){
                     this.update_style(i);
                     this.add_situation(this.situations[i],i);
    ...
    else if (i == this.situations.length){
        $('#refresh_sits').html('');
    }
}
/*
 * CKharitonov
 */
SIT_SCOPE.refresh_content = function(){
    var html = '<div class="alert alert-info" style="text-align:center;margin-bottom:0px;margin-top:2px;cursor:pointer;" onclick="SIT_SCOPE.refresh_situations()">';
        html += '<span><i class="fa fa-repeat" aria-hidden="true"></i></span> Показать еще';
    html += '</div>';
    $('#refresh_sits').html(html);
}
/*
 * CKharitonov
 */
SIT_SCOPE.situation_view = function(){
    ...
    this.refresh_situations();

    if (this.situations.length > 10){
        for (var i=9; i<this.situations.length; i++){
            this.update_style(i);
            this.load_from_db(this.situations[i],i);
...
SIT_SCOPE.event_onmousedown = function(evt_id){
    ...
    if (SIT_SCOPE.events[evt_id]["fk_situation"] != null){
        if ($('[sit_id = '+SIT_SCOPE.events[evt_id]["fk_situation"]+']').attr('sit_is_fini') == 'T'){
            return;
        }
    }
 ...
 * CKharitonov
 */
SIT_SCOPE.group_onmousedown = function(div_id,e){
    $('#pop_win').css({
        'visibility': 'hidden'
    });
    if (SIT_SCOPE.check_is_finished_situation(e)){
        return;
    }
 ...
 * CKharitonov
 */
SIT_SCOPE.group_onmouseup = function(div_id,e){
    ...
    if (SIT_SCOPE.events[SIT_SCOPE.selected_objects[0]]["fk_copy"] != null){
        if (SIT_SCOPE.group_events[SIT_SCOPE.events[SIT_SCOPE.selected_objects[0]]["fk_copy"]].div_id != div_id){
            if (SIT_SCOPE.check_is_finished_situation(e)){
                alert('Действие невозможно! Ситуация завершена.');
                return;
            }
...
SIT_SCOPE.load_mouse_events = function(){
    ...
    document.getElementById('move_left').onclick = function(){ move(1); };
    document.getElementById('move_right').onclick = function(){ move(-1); };
    $('#sit_cont').unbind();
...
function on_group_mouseup(sit_id,sit_count,e){
...
SIT_SCOPE.add_group_button = function(grp,sit_id){
    ...
    var html = '<span class="group" id="cont_'+grp.div_id+'" data-sort="'+grp.group_order+'"><div id="'+CFUtil.guid()+'" sit="'+sit_id+'" pos="" class="for_drag">&nbsp;</div>';
        html += '<span class="well well-sm" onmouseup="SIT_SCOPE.group_onmouseup(\''+grp.div_id+'\',this)" ';
            html += 'onmousedown="SIT_SCOPE.group_onmousedown(\''+grp.div_id+'\',this)" ';
 ...
 * CKharitonov
 */
SIT_SCOPE.add_ojur_rec_button = function(rec,sit_id){
	if ($('#'+rec.id+'_btn').is()) return;
    var html = '<span id="'+rec.id+'_btn" onclick="SIT_SCOPE.focus_on_message(\''+rec.id+'\');" title="Сообщение из ОЖУР">';
        html += '<span class="well well-sm">';
            html += '<span>'+SIT_SCOPE.check_for_alarm(rec)+'</span>';
            html += '<span style="margin-right:5px;">'+rec.task_name+'</span>';
        html += '</span>';
    html += '</span>';
    $('#ojur_recs_'+sit_id).append(html);
}
/*
 * CKharitonov
 */
...
SIT_SCOPE.focus_on_message = function(msg_id){
	//проверка на существование в timeline сообщения с указанным ID
	if(typeof(SIT_SCOPE.timeline.itemsData._data[msg_id]) == 'undefined') {
		alert('Нет привязанного сообщения к выбранному событию');
		return;
	}
...
SIT_SCOPE.set_event_content = function(evt){
    ...
    for (var j=0; j<evt.length; j++){
        ...
        html += ' id="'+evt[j]["id"]+'"';
        html += ' title="'+title+'">'
        + '<span class="glyphicon glyphicon-info-sign" style="cursor:pointer;" onclick="SIT_SCOPE.open_event_window('+evt[j]["id"]+')"></span>'
        + '<span onclick="SIT_SCOPE.select_object('+evt[j]["id"]+')" onmousedown="SIT_SCOPE.event_onmousedown(\''+evt[j]["id"]+'\')">&nbsp;'+content+'</span>'
        + '</span>&nbsp;&nbsp;';
    }
    return html;
}
...
SIT_SCOPE.set_message_content = function(msg){
    ...
    var html = '<div style="max-width:'+SIT_SCOPE.msg_window_width+';min-width:'+SIT_SCOPE.msg_window_width+';max-height:'+SIT_SCOPE.msg_window_height+';">';
        html += '<table class="table" style="font-size:12px;margin-bottom:0;" width="'+SIT_SCOPE.msg_window_width+'">';
            html += '<tbody>';
                html += '<tr>';
                    ...
                    html += '<td>';
                        html += '<b>'+check_not_null(msg.semantic_id)+'</b><br>';
                        if (msg.msg_type == 'ojur'){
                            html += SIT_SCOPE.check_for_alarm(msg);
                        }
                        html += content;
                    html += '</td>';
                html += '</tr>';
...
SIT_SCOPE.load_data = function(){
    ...
    if (SIT_SCOPE.messages && SIT_SCOPE.messages.length != 0){
        for (var i=0; i<SIT_SCOPE.messages.length; i++){
            try {
                items.push({
    ...
    var midpoint = new Date((start_date.getTime() + end_date.getTime()) / 2);
    start_date = new Date(midpoint.setHours(midpoint.getHours()-6));
    end_date = new Date(midpoint.setHours(midpoint.getHours()+12));
 ...
 * CKharitonov
 */
SIT_SCOPE.set_style = function(){
    if (!SIT_SCOPE.situations) return;
...
SIT_SCOPE.load_from_db = function(sit,i){
    ...
    for (var val in tmp_group_events){
        var data = new Array();
        var group_id = null;
        var group_name = '';
        var group_order = null;
        ...
        grp.div_id = CFUtil.guid();
        var sit_id = SIT_SCOPE.situations[i].events[0].fk_situation;
        grp.events = data;
        SIT_SCOPE.group_events[group_id] = grp;
        SIT_SCOPE.add_group_button(grp,sit_id);
    }
...
SIT_SCOPE.run_auto_update = function(){
    return;
    clearInterval(SIT_SCOPE.interval_id);
    SIT_SCOPE.interval_id = setInterval(
        function(){
...
SIT_SCOPE.run_auto_ajax_update = function(){
    ...
    $.ajax({
        ...
        type: "POST"
          ...
          try {
            ...
            SIT_SCOPE.error_window(response);
            return;
        }
        if (data.length == 0 ){
            return;
        ...
        if (data && data.situations){
            for (var i=0; i<data.situations.length; i++){
                if (!(in_array(data.situations[i].id,SIT_SCOPE.sits_id_list))){
                    var clss = 'body{overflow: hidden;}.label-group'+SIT_SCOPE.situations.length+' { background-color: '+randomColor({luminosity:'dark'})+'; cursor: pointer; } ';
                    $('#style').append(clss);
                    SIT_SCOPE.sits_id_list.push(data.situations[i].id);
                    SIT_SCOPE.situations.push(data.situations[i]);

                    if (($('#sit_cont [sit_id]').length % 2) != 0 || $('#sit_cont [sit_id]').length == 0){
                        var key = String($('#txt_filter').val()).toUpperCase();
                        var keys = key.split(',');
                        if (key != '' && key.length > 1){
                            for (var k in keys){
                                if (keys[k].length > 1){
                                    if (String(SIT_SCOPE.situations[i]["text"]).toUpperCase().indexOf(keys[k]) != -1 && $('div[sit_id="'+SIT_SCOPE.situations[i].id+'"]').length == 0){
                            ...
                            var bits = check_filter_values();
                            if (bits.ans == SIT_SCOPE.situations[i].is_auto_reg || bits.tns == SIT_SCOPE.situations[i].is_current || bits.zns == SIT_SCOPE.situations[i].is_finished){
                                SIT_SCOPE.add_situation(data.situations[i],SIT_SCOPE.situations.length-1);
                            }
                        }
                    }
                    else {
                        SIT_SCOPE.refresh_content();
                    }
                }
            }
        }
        ...
        if (data && data.messages){
            for (var i=0; i<data.messages.length; i++){
                if (!(in_array(data.messages[i].id,SIT_SCOPE.msgs_id_list))){
                    ...
                    if (data.messages[i].msg_type == 'ojur'){
                        SIT_SCOPE.ojur_recs.push(data.messages[i]);
                        if (data.messages[i].fk_situation != ''){
                            SIT_SCOPE.add_ojur_rec_button(data.messages[i],data.messages[i].fk_situation);
                        }
                    }
                    SIT_SCOPE.add_message(data.messages[i]);
                    if (data.messages[i].events && data.messages[i].events.length != 0){
                        for (var j=0; j<data.messages[i].events.length; j++){
                            if (data.messages[i].events[j].fk_situation != null){
                                if (SIT_SCOPE.group_events[data.messages[i].events[j].id] == undefined && data.messages[i].events[j].fk_copy == data.messages[i].events[j].id){
                                    var grp = new group_event();
                                    grp.group_id = data.messages[i].events[j].id;
                                    grp.group_name = data.messages[i].events[j].type;
                                    for (var n=0; n<SIT_SCOPE.situations.length; n++){
                                        if (SIT_SCOPE.situations[n].id == data.messages[i].events[j].fk_situation){
                                            grp.class_name = 'label-group'+n;
                                            if ($('#'+data.messages[i].events[j].id).hasClass(SIT_SCOPE.default_class)){
                                                $('#'+data.messages[i].events[j].id).removeClass(SIT_SCOPE.default_class).addClass('label-group'+n);
                                            }
                                        }
                                    }
                                    grp.group_order = data.messages[i].events[j].event_order;
                                    grp.div_id = CFUtil.guid();
                                    var sit_id = data.messages[i].events[j].fk_situation;
                                    grp.events.push(data.messages[i].events[j]);
                                    SIT_SCOPE.group_events[data.messages[i].events[j].id] = grp;
                                    SIT_SCOPE.add_group_button(grp,sit_id);
                                }
                            }
                        }
		...
		var diff = $(SIT_SCOPE.msgs_id_list).not(data.msgs_id_list).get();

        if (diff){
            for (var i=0; i<diff.length; i++){
                if (diff[i].substring(0,4) == 'ojur'){
                    SIT_SCOPE.remove_ojur_rec(diff[i]);
                }
            }
        }
    ...
    $('#sits_count').html('Количество ситуаций: '+SIT_SCOPE.situations.length+'.');
    $('#msgs_count').html('Количество сообщений: '+SIT_SCOPE.messages.length+'.');
}
/*
 * CKharitonov
 */
...
SIT_SCOPE.add_situation = function(sit,i){
        ...
        html += '<table class="table" style="font-size:12px;">';
            html += '<tr>';
                ...
                html += '<td style="vertical-align:middle;">';
                	html += '<span id="ojur_recs_'+sit["id"]+'"></span>';
                    html += '<span id="events_'+sit["id"]+'"></span>';
                    if (String(sit["is_finished"]).toUpperCase() == 'F'){
                            html += '<span class="well-sm fbadge"';
                                html += 'onmouseup="on_group_mouseup('+sit["id"]+','+i+',this)" ';
    ...
    $('#sit_cont').append(html);
    //popover_init
    SIT_SCOPE.load_from_db(sit,i);//при первой загрузке уже выполняется function situation_view

    for (var j=0; j<SIT_SCOPE.ojur_recs.length; j++){
    	if (SIT_SCOPE.ojur_recs[j].fk_situation == sit["id"]){
    		SIT_SCOPE.add_ojur_rec_button(SIT_SCOPE.ojur_recs[j],SIT_SCOPE.ojur_recs[j].fk_situation);
    	}
	}

}

SIT_SCOPE.add_html_sit_cont = function(html){
    $('#sit_cont').append(html);
}

SIT_SCOPE.popover_init = function(){
    $('.pop').popover({html:true});
}
 ...
 * UPD: Artem
 */
SIT_SCOPE.update_style = function(i_grouop){
    var clss = '';
    /*
    if (SIT_SCOPE.situations != null){
        for (var i=0; i<SIT_SCOPE.situations.length; i++){
            clss += 'body{overflow: hidden;}.label-group'+i+' { background-color: '+randomColor({luminosity:'dark'})+'; cursor: pointer; } ';
        }
    }
    $('#style').append(clss);
    */
    if (i_grouop || i_grouop == 0){
        clss= ' .label-group'+i_grouop+' { background-color: '+randomColor({luminosity:'dark'})+'; cursor: pointer; } ';
    }
    $('#style').append(clss);
}
...
SIT_SCOPE.get_option_buttons = function(sit,mode){
    var btn_set = '<div style="cursor:default;">';
        ...
        var bell = '';
        if (sit.is_alarm == 't' && sit.is_acquainted_alarm != 't'){
            bell = '<span class="glyphicon glyphicon-bell alarm" style="font-size:15px;top:4px;cursor:pointer;" title="Аларм" onclick="SIT_SCOPE.confirmation(\'situation\','+sit["id"]+')"></span>&nbsp;';
        }
        if (sit.is_alarm == 't' && sit.is_acquainted_alarm == 't'){
            bell = '<span class="glyphicon glyphicon-bell acknowledged_alarm" style="font-size:15px;top:4px;" title="Квитированный Аларм"></span>&nbsp;';
        }
        btn_set += bell;
        if (sit["name"]){
    ...
    btn_set += '<div class="btn-group">';
    	var is_ojur_recs = false;
	    for (var i=0; i<SIT_SCOPE.ojur_recs.length; i++){
	    	if (SIT_SCOPE.ojur_recs[i].fk_situation == sit["id"]){
	    		is_ojur_recs = true;
	    	}
		}
        if (sit.events.length == 0 && sit["is_finished"] != 't' && !is_ojur_recs){
            btn_set += '<button type="button" class="btn btn-default btn-xs" onclick="SIT_SCOPE.remove_situation(\''+sit["id"]+'\')" title="'+langs.get_term('txt_delete_situation')+'">&nbsp;<span class="glyphicon glyphicon-remove alert-danger"></span>&nbsp;</button>';
        }
        if (sit["is_finished"] != 't'){
        ...
        if (sit["is_finished"] != 't'){
            btn_set += '<button type="button" title="'+langs.get_term('txt_close_situation')+'" class="btn btn-default btn-xs" onclick="SIT_SCOPE.close_situation(\''+sit["id"]+'\')">&nbsp;<span class="glyphicon glyphicon-stop alert-danger"></span>&nbsp;</button>';
        }
        if (sit["is_finished"] == 't'){
            btn_set += '<i id="act_'+sit["id"]+'">';
                if (sit["act_idn"] == null){
                    btn_set += '<button type="button" title="Прикрепить акт?" class="btn btn-default btn-xs" onclick="SIT_SCOPE.open_acts_list(\''+moment(sit["registration_datetime"]).format("YYYY-MM-DD")+'\',\''+sit["id"]+'\')">&nbsp;<span style="color:#ffbfbf;"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i></span>&nbsp;</button>';
                }
                else {
                    btn_set += '<button type="button" class="btn btn-default btn-xs pop" id="act_popover_'+sit["id"]+'" data-container="body" data-toggle="popover" data-placement="right" data-content="" onclick="SIT_SCOPE.add_act_popover_content(\''+sit["id"]+'\',\''+sit["act_idn"]+'\')">&nbsp;<span style="color:#a7f5a7;"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i></span>&nbsp;</button>';
                }
            btn_set += '</i>';
        }
    btn_set += '</div>&nbsp;&nbsp;&nbsp;';
...
SIT_SCOPE.add_popover_content = function(sit_id){
    var html = '<table class="table table-hover" style="font-size:12;margin-bottom:0px;">';
        html += '<tbody>';
            html += '<tr onclick="SIT_SCOPE.open_logic1(\''+sit_id+'\')" style="cursor:pointer;"><td><span style="color: #71c372;"><i class="fa fa-random" aria-hidden="true"></i></span>  ЭСШ-Логика</td></tr>';
            html += '<tr onclick="SIT_SCOPE.open_event(\''+sit_id+'\')" style="cursor:pointer;"><td><span style="color: #71c372;"><i class="fa fa-check-square-o" aria-hidden="true"></i></span>  ЭСШ-События</td></tr>';
            html += '<tr onclick="SIT_SCOPE.open_time(\''+sit_id+'\')" style="cursor:pointer;"><td><span style="color: #71c372;"><i class="fa fa-clock-o" aria-hidden="true"></i></span>  '+langs.get_term('txt_dss_time_short')+'</td></tr>';
            html += '<tr onclick="SIT_SCOPE.open_radec(\''+sit_id+'\')" style="cursor:pointer;"><td><span style="color: #71c372;"><i class="fa fa-repeat" aria-hidden="true"></i></span>  '+langs.get_term('txt_dss_cycle_short')+'</td></tr>';
        html += '</tbody>';
    html += '</table>';
...
SIT_SCOPE.get_message_buttons = function(msg_id){
    var btn_set = '';
    btn_set += '<span class="label label-default" style="cursor:pointer;" title="'+langs.get_term('txt_open_msg_info')+'" onclick="SIT_SCOPE.message_info_view(\''+msg_id+'\')"><span><i class="fa fa-folder-open" aria-hidden="true"></i></span></span><br>';
    return btn_set;
}
/*
 * CKharitonov
...
SIT_SCOPE.update_data = function(btn){
    ...
    SIT_SCOPE.load_mouse_events();
    SIT_SCOPE.load_mouse_events_for_drag();
    history.pushState({},"",SIT_SCOPE.base_url+'index.php/conframe_bi/sit_scope_v3/'+$('#d_from').val()+'/'+$('#d_to').val());
...
SIT_SCOPE.update_sit_scope_data = function(btn){
    $.ajax({
        ...
        success: function(data){
            ...
            SIT_SCOPE.events = data['events'];
            SIT_SCOPE.ojur_recs = data['ojur_recs'];
            //SIT_SCOPE.alarm_rules = data['alarm_rules'];
            $('#style').html('');
          //  SIT_SCOPE.update_style();
            $('.pop').popover({html:true});
            SIT_SCOPE.update_data(btn);
            $('#sit_cont').html('');

            SIT_SCOPE.refresh_situations();
            $('#selected_events_window').dialog('close');
            SIT_SCOPE.selected_objects = new Array();
            //SIT_SCOPE.run_auto_update();
            $('#sits_count').html('Количество ситуаций: '+SIT_SCOPE.situations.length+'.');
            ...
            for (var i=0; i<SIT_SCOPE.messages.length; i++){
                if (SIT_SCOPE.messages[i].id == msg_id){
                    html += '<table class="table table-condensed table-striped" style="font-size:12px;">';
                        html += '<tr>';
                            html += '<td width="30%"><b>'+langs.get_term('txt_from')+':</b></td>';
                        ...
                        html += '<tr>';
                            ...
                            html += '<td>'+
                                    moment(CFUtil.get_local_datetime(SIT_SCOPE.messages[i].date_time)).format("DD.MM.YYYY HH:mm:ss")+'</td>';
                        html += '</tr>';
                        html += '<tr>';
                            if (SIT_SCOPE.messages[i].msg_type == 'ojur'){
                                html += '<td><b>Семантический идентификатор ситуации:</b></td>';
                                if (SIT_SCOPE.messages[i].semantic_id != null){
                                    html += '<td>'+SIT_SCOPE.messages[i].semantic_id+' <span class="glyphicon glyphicon-search" style="cursor:pointer" onclick="search_situation(\''+SIT_SCOPE.messages[i].semantic_id+'\');" title="'+langs.get_term('txt_show_situation')+'"></span></td>';
                                }
                                else {
                                    html += '<td> - </td>';
                                }
                        ...
                        html += '<tr>';
                            ...
                            html += '<td>';
                                if (SIT_SCOPE.messages[i].msg_type == 'ojur'){
                                    html += SIT_SCOPE.check_for_alarm(SIT_SCOPE.messages[i]);
                                }
                                html += check_not_null(SIT_SCOPE.messages[i].text);
                            html += '</td>';
                        html += '</tr>';
                        if (SIT_SCOPE.messages[i].msg_type != 'ojur'){
                            html += '<tr>';
...
SIT_SCOPE.open_event_window = function(evt_id){
    ...
    if (dialog){
        var html = '<div id="evt_info" style="max-height:450px;min-height:270px;overflow:auto;"></div>';
...
SIT_SCOPE.event_info_view = function(evt_id,mode){
    $.ajax({
        ...
        beforeSend: function(){
            $('#'+mode).html(SIT_SCOPE.start_preloader());
        },
        success: function(data){
            try {
                var data = JSON.parse(data);
            ...
            var html = '<table class="table table-condensed table-hover" style="font-size:12px;margin-bottom:0px;">';
                html += '<tbody>';
                    ...
                    html += put_array_of_attributes(langs.get_term('txt_result_avr'),opened_event["result_avr"]);
                    html += put_array_of_attributes(langs.get_term('txt_result_rpv'),opened_event["result_rpv"]);
                    html += put_array_of_attributes(langs.get_term('txt_accident_fact'),opened_event["accident_fact"]);
                html += '</tbody>';
            html += '</table>';
            $('#'+mode).html(html);
        }
...
function check_filter_values(){
    var data = {
        'ans' : ($('#ans').hasClass('btn-primary')) ? 't' : 'f',
        //'rns' : $('#rns').hasClass('btn-primary'),
        'tns' : ($('#tns').hasClass('btn-primary')) ? 't' : 'f',
        'zns' : ($('#zns').hasClass('btn-primary')) ? 't' : 'f'
    };
    var check = false;
    for (key in data){
        if (data[key] == 't'){
    ...
    if (dialog){
        var html = $.ajax({
            ...
            $('#btn_save').click(function(){
                for (var i=0; i<SIT_SCOPE.situations.length; i++){
                    if (SIT_SCOPE.situations[i].id == sit_id){
                        if ($('#is_current :selected').val() == 1){
                            SIT_SCOPE.situations[i].is_current = 't';
                            $('[sit_id = '+sit_id+']').attr('sit_is_curr','T');
                        }
                        else if ($('#is_current :selected').val() == 0){
                            SIT_SCOPE.situations[i].is_current = 'f';
                            $('[sit_id = '+sit_id+']').attr('sit_is_curr','F');
                        }
...
/*
 * TSRuban
 * Открытие списка datatables всех ситуаций из БД
 */
...
SIT_SCOPE.check_group_alarm = function(event){
    ...
    for (var i=0; i<event.length; i++){
        if (event[i].is_alarm == 't' && event[i].is_acquainted_alarm != 't'){
            html = '<span class="alarm" title="Аларм" style="font-size:15px;top:4px;margin-right:3px;"><i class="fa fa-bell" aria-hidden="true"></i></span>';
            return html;
        }
        if (event[i].is_alarm == 't' && event[i].is_acquainted_alarm == 't'){
            html = '<span class="acknowledged_alarm" title="Квитированный Аларм" style="font-size:15px;top:4px;margin-right:3px;"><i class="fa fa-bell" aria-hidden="true"></i></span>';
        }
    }
    return html;
}
...
SIT_SCOPE.check_for_alarm = function(event){
    if (!event) return '';
    var html = '';
    if (event.is_alarm == 't' || event.is_alarm == 'true'){
        html = '<span class="glyphicon glyphicon-bell alarm" style="cursor:pointer;" title="Аларм"';
            if (!event.msg_type || event.msg_type != 'ojur'){
                html += ' onclick="SIT_SCOPE.confirmation(\'event\','+event.id+')"';
            }
        html += '></span>&nbsp;';
    }
    if (event.is_acquainted_alarm == 't' || event.is_acquainted_alarm == 'true'){
        html = '<span class="acknowledged_alarm" title="Квитированный Аларм"><i class="fa fa-bell" aria-hidden="true"></i></span>&nbsp;';
    }
    return html;
}
/*
...
SIT_SCOPE.confirmation = function(type,id){
    ...
    if (dialog){
        var html = '<div class="form-group input-group-sm" id="for_is_acquainted_alarm">';
            ...
            html += '</select>';
            html += '<br>';
            html += '<button type="button" class="btn btn-primary btn-sm" id="btn_save" onclick="SIT_SCOPE.update_socket(\''+type+'\','+id+')">Сохранить</button>&nbsp;&nbsp;';
 ...
 * CKharitonov
 */
SIT_SCOPE.update_socket = function(type,id){
    $.ajax({
        url: SIT_SCOPE.ojur_update_socket,
        type: 'POST',
        data: {
		"alarm_acknowledg_record" : JSON.stringify({ "record_id" : id, "ojur_id": 12, "rec_type": type }),
		"session_hash": localStorage["session_hash"],
		"session_login": localStorage["session_login"],
		"session_num": localStorage["session_num"]
		},
        success: function(data){
            //Не удалось квитировать аларм
            if (String(data).trim() != 'acknowledg'){
                alert("Вы не можете квитировать данный аларм!");
            }
            else {
                SIT_SCOPE.update_alarm(type,id);
            }
...
SIT_SCOPE.update_alarm = function(type,id){
    if (type == 'situation' && $('#is_acquainted_alarm').val() == 1){
        for (var i=0; i<SIT_SCOPE.situations.length; i++){
            if (SIT_SCOPE.situations[i].id == id){
                if (SIT_SCOPE.situations[i].is_alarm == 't'){
                    SIT_SCOPE.situations[i].is_acquainted_alarm = 't';
                }
                for (var j=0; j<SIT_SCOPE.situations[i].events.length; j++){
                    if (SIT_SCOPE.situations[i].events[j].is_alarm == 't'){
                        ...
                        $('#'+SIT_SCOPE.group_events[SIT_SCOPE.situations[i].events[j].fk_copy].div_id+'_alarm').html('<span class="glyphicon glyphicon-bell acknowledged_alarm" title="Квитированный Аларм" style="font-size:15px;top:4px;"></span>&nbsp;');
    ...
    else if (type == 'event' && $('#is_acquainted_alarm').val() == 1){
            ...
            success: function(data){
                ...
                for (var i=0; i<SIT_SCOPE.situations.length; i++){
                    if (SIT_SCOPE.situations[i].id == sit_id){
                        ...
                        var html = '';
                        if (act_id == null){
                            html += '<button type="button" title="Прикрепить акт?" class="btn btn-default btn-xs" onclick="SIT_SCOPE.open_acts_list(\''+moment(SIT_SCOPE.situations[i].registration_datetime).format("YYYY-MM-DD")+'\',\''+sit_id+'\')">&nbsp;<span class="glyphicon glyphicon-warning-sign" style="color:#ffbfbf;"></span>&nbsp;</button>';
                        }
                        else {
 ...
 *
 */
SIT_SCOPE.open_alarm_list = function(){
    var dialog = CFUtil.dialog.create("alarm_list",
    {
        title: 'Список алармов',
        autoOpen: false,
        height: $(window).height(),
        width: $(window).width()-500
    });
    if (dialog){
        html = $.ajax({
            url: SIT_SCOPE.base_url+"index.php/qcore/ajax/load_table/conframe_bi/event_alarm",
            type: "POST"
        }).done(function (response, textStatus, jqXHRб){
            $(dialog).html('<div >'+response+'</div>');
        });
 ...
 * CKharitonov
 */
SIT_SCOPE.start_preloader = function(){
 ...
 * CKharitonov
 */
SIT_SCOPE.check_is_finished_situation = function(e){
    var found_elem = false;
    $(e).parents().map(function(){
        if ($(this).attr('sit_is_fini') == 'T'){
            found_elem = true;
            return;
        }
    });
 ...
 * CKharitonov
 */
SIT_SCOPE.error_window = function(data){
    var dialog = CFUtil.dialog.create("error_window",
    {
        title: 'Ошибка',
        autoOpen: false,
        height: 'auto',
 ...
 * CKharitonov
 */
SIT_SCOPE.remove_ojur_rec = function(rec_id){
	SIT_SCOPE.items.remove(rec_id);
	$('#'+rec_id+'_btn').remove();
	for (var i=0; i<SIT_SCOPE.msgs_id_list.length; i++){
		if (SIT_SCOPE.msgs_id_list[i] == rec_id){
			SIT_SCOPE.msgs_id_list.splice(i,1);
		}
	}
	for (var i=0; i<SIT_SCOPE.messages.length; i++){
		if (SIT_SCOPE.messages[i].id == rec_id){
			SIT_SCOPE.messages.splice(i,1);
		}
	}
}
