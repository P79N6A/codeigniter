...
BEOM_BROWSER.load_data = function(idx){
	...
	*/
    ...
    BEOM_BROWSER.network.on('oncontext',function (properties){
        if ( (properties.nodes.length > 0) ){
			generate_context(properties);
        }
    });
...
BEOM_BROWSER.add_nodes = function(parent){
    ...
    var edges = [];
    for (var i=0; i<BEOM_BROWSER.data.length; i++){
        if (BEOM_BROWSER.data[i]["parent"] == parent && BEOM_BROWSER.data[i]["is_hidden"] != 't'){
            nodes.push({
...
BEOM_BROWSER.show_data_subject = function(){
    ...
    query.done(function (response, textStatus, jqXHRб){
        ...
        html_text = '<table class="table table-condensed" style="font-size:12px; color: #333 !important">';
        /*for (option in data ){
