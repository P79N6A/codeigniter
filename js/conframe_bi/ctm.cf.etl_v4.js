...
CFbi.window_situation = function(node){
	if (!CFbi.events_info || CFbi.events_info == "null" || !node || !node.full_data) return;
    ...
    if (CFbi.message_info){
        ...
        $(CFbi.message_info).html(html);
        CF_Events_info.get_event_info(node.event_id_db,'info_'+id_inf);
        //if (!node.full_data) return;
        if (node.full_data.mas_copy){
            for (var mc in node.full_data.mas_copy){
                CF_Events_info.get_event_info(node.full_data.mas_copy[mc].event_id_db,'info_'+id_inf);
            }
        }
		...
		CFbi.network.on("click",function(e){
			...
			if (e.nodes.length != 0){
				for (var i in CFbi.nodes){
					if (e.nodes[0] == CFbi.nodes[i].id && (CFbi.nodes[i].group == "object" || CFbi.nodes[i].group == "group_object")){
...
CFbi.set_etl_schema_v2 = function(){
	...
	for (var i in all_events){
		...
		if (all_events[i].mas_copy) delete all_events[i].mas_copy;

        if (all_events[i].fk_copy != "" && all_events[i].fk_copy && (all_events[i].fk_copy != all_events[i].id)){
            copy.push(all_events[i]);
            continue;
        };
		object = add_object(all_events[i]);//получили параметры обьекта
		object.full_data = all_events[i];
		CFbi.nodes.push(object);//
	...
	for (var c in copy){
        ...
        for (var s in CFbi.nodes){
            if (CFbi.nodes[s].full_data.event_id_db == copy[c].fk_copy){
                if (!CFbi.nodes[s].full_data.mas_copy) CFbi.nodes[s].full_data.mas_copy = [];
                CFbi.nodes[s].full_data.mas_copy.push(copy[c]);
                mc = CFbi.nodes[s].full_data.mas_copy.length;
                mc++;
                CFbi.nodes[s].label  = CFbi.nodes[s].full_data.semantic_id + '('+mc+')';
            	CFbi.nodes[s].group = 'group_object';
                break;
            }
        }
