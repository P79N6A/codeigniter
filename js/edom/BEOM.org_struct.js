...
Mtt.init = function(){
    ...
    for (i in Mtt.structure){
        nodes.push({
            ...
            control_level: Mtt.structure[i]["os_type"],
            group: 'switch',
            is_open: false
        });
    }
    ...
    function generate_discription(properties){
		var html = '<div id="pop_up_window" class="pop_up_window field_description" style="position:absolute;left:'+(properties.pointer.DOM.x+25)+'px;top:'+(properties.pointer.DOM.y+5)+'px;">'+Mtt.nodes._data[properties.nodes[0]].title+'<br>(Тип орг.структуры: '+Mtt.nodes._data[properties.nodes[0]].control_level+')</div>';
        $('body').append(html);
    }
...
Mtt.add_task = function(){
	...
	if ( dialog ){
		html = $.ajax({     
			url: Mtt.base_url+"index.php/qcore/ajax/load_form/edom/org_struct/",    
			type: "POST"         
...
Mtt.edit_task = function(){
	...
	if ( dialog ){
		html = $.ajax({
			url: Mtt.base_url+"index.php/qcore/ajax/edit_form/edom/org_struct/" + choose_id_obj,    
			type: "POST"
		}).done(function (response, textStatus, jqXHRб){
			$(dialog).html(response);
		});
...
Mtt.remove_task = function(){
	...
	if(confirm("Вы действительно хотите удалить объект?")){
		$.ajax({
			url: Mtt.base_url+"index.php/edom/remove_org_struct/" + choose_id_obj,
			type: "POST",
			data: {id:choose_id_obj}
		}).done(function (response, textStatus, jqXHRб){
			delete_flag = JSON.parse(response);
			if(Mtt.structure[choose_id_obj].parent == '0') load_page('index.php/edom/org_struct/');
...
Mtt.update = function (obj){
	...
	if(choose_id_obj == 0){
		load_page('index.php/edom/org_struct/' + obj.uuid);
	}
...
show_name_root = function(){
	...
	if(array_keys.length > 0){
		$('#name_root').html(Mtt.structure[array_keys[0]]['label']);
	}
}